...
This document describes how to use a server environment variable to store the key used to encrypt and decrypt text with _Crypt for WordPress_.
...
Or use https://github.com/vlucas/phpdotenv to work with .env files. See the instructions there for the setup.

## Usage

1. Set up a server-side variable with the key of your choice.
2. In the configuration for loading _Crypt for WordPress_ via `set_config()`, set the following values:
